...
def pred_traj(ncomb,b_xfc,b_yfc,b_zfc,rho,a,d_ij,alpha_ij,beta_ij,lambda_xij,lambda_yij,lambda_zij,Afc,P,beq_x,beq_y,beq_z,cost_mat_inv_x,cost_mat_inv_y,cost_mat_inv_z):
    ...
    c2_d = (lambda_xij*np.cos(alpha_ij)*np.sin(beta_ij) + lambda_yij*np.sin(alpha_ij)*np.sin(beta_ij) + lambda_zij*np.cos(beta_ij)+ rho*xij*np.cos(alpha_ij)*np.sin(beta_ij) + rho*yij*np.sin(alpha_ij)*np.sin(beta_ij) +rho*zij*np.cos(beta_ij))
...
for step in range(planning_iters):
    ...
    beq_z = np.vstack((z[:,1],vz_pred[:,1],az_pred[:,1],z_fin,vz_fin,az_fin)).T
